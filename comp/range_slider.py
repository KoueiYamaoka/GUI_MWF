...
class SliderThumb:
    ...
    def SetPosition(self, pos):
        ...
        if who_other == "low":
            pos_x = max(
                other_pos[0] + other_thumb.size[0] / 2 + self.size[0] / 2, pos_x
            )
        else:
            pos_x = min(
                other_pos[0] - other_thumb.size[0] / 2 - self.size[0] / 2, pos_x
    ...
    def OnPaint(self, dc):
        ...
        # Draw text (*)
        if self.value >= self.parent.GetMax() // 2:
            dc.DrawText(
                "{:.2f}".format(self.value / 1000), my_pos[0] - 30, my_pos[1] - 50
            )
        else:
            dc.DrawText(
                "{:.2f}".format(self.value / 1000), my_pos[0] - 10, my_pos[1] - 50
            )
...
class RangeSlider(wx.Panel):
    def __init__(
        ...
        self.background_color = parent.GetBackgroundColour()  # (*)
    ...
    def OnPaint(self, evt):
        ...
        background_brush = wx.Brush(self.background_color, wx.SOLID)  # (*)
        ...
        dc.DrawRectangle(
            self.border_width,
            h / 2 - track_height / 2,
            w - 2 * self.border_width - right_margin,  # (*)
            track_height,
        )
        # Draw selected range
        if self.IsEnabled():
        ...
        dc.DrawRectangle(
            low_pos, h / 2 - track_height / 4, high_pos - low_pos, track_height / 2
        )
        # Draw thumbs
        for thumb in self.thumbs.values():
            thumb.OnPaint(dc)
        ...
        # Draw text (*)
        dc.DrawText(str(self.min_value) + " kHz", 0, h / 2 + 20)
        dc.DrawText(str(self.max_value // 1000) + " kHz", w - 62, h / 2 + 20)
...
class TestFrame(wx.Frame):
    ...
    def rangeslider_changed(self, evt):
        ...
        self.rangeslider_static.SetLabel(
            "Low value: {:.0f}, High value: {:.0f}".format(lv, hv)
        )
